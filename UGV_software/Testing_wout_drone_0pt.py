...
async def search_alg(offboard,time_detected, time_lost_threshold, t_mission_start, time_4_mission, step, current_angle, max_angle_down, default_angle, yaw):
...
async def correct_heading(rover, target_yaw, threshold=5):
    """
    Perform a zero-point turn to correct heading error until it is within the threshold.
    :param rover: The rover object.
    :param target_yaw: Desired yaw angle in degrees.
    :param threshold: Acceptable heading error in degrees.
    """
    print(f"Correcting heading to {target_yaw} degrees...")
    async for telemetry in rover.telemetry.attitude():
        ...
        # Perform zero-point turn if heading error exceeds threshold
        while abs(heading_error) > threshold:
            turn_speed = 50 if heading_error > 0 else -50  # Positive or negative turn speed
            velocity_command = VelocityBodyYawspeed(0.0, 0.0, 0.0, turn_speed)
            await rover.offboard.set_velocity_body(velocity_command)
        ...
        await rover.offboard.set_velocity_body(velocity_command)
        print(f"Heading corrected to within {threshold} degrees of {target_yaw}.")
        break
...
async def main():
    ...
    forward_velocity = 0
    lateral_velocity = 0
    TARGET_LATITUDE = 34.4188996
    TARGET_ALTITUDE = 0
    TARGET_YAW = 0
    TARGET_LONGITUDE = -119.8553301
    count = 0
    ...
    # Correct heading before navigating to waypoint
    await correct_heading(rover, TARGET_YAW)

    print(f"-- Driving rover to waypoint: lat={TARGET_LATITUDE}, lon={TARGET_LONGITUDE}")
    await rover.action.goto_location(TARGET_LATITUDE, TARGET_LONGITUDE, TARGET_ALTITUDE, TARGET_YAW)
