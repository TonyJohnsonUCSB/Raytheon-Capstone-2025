...
picam2 = Picamera2()
cfg = picam2.create_preview_configuration(main={"format":"RGB888", "size":RESOLUTION})
    ...
    while True:
        ...
        if ids is not None:
            for idx, mid in enumerate(ids.flatten()):
                ...
                # NED in cm
                N =  y_north * METERS_TO_CM
                E =  x_east  * METERS_TO_CM
                D =  z_down  * METERS_TO_CM

                # GPS of marker
                lat, lon, alt = enu_to_geodetic(x_east, y_north, -z_down)

                # overlays
                cv2.putText(stab,
                    f"N:{N:.1f}cm E:{E:.1f}cm D:{D:.1f}cm",
